...
staged_files = [namestat[0] for namestat in Popen(['git', 'diff', '--staged', '--name-status'],
...
nb_untracked = len([0 for status in Popen(['git', 'status', '--porcelain'], stdout=PIPE)
...
if not branch:  # not on any branch
    ...
    if remote_name:
        ...
        if revgit.poll():  # fallback to local
            revlist = Popen(['git', 'rev-list', '--left-right', '%s...HEAD' % merge_name],
